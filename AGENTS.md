...
- **Delivery is a public GitHub Release, not a local file.** `v1.0.0` (tag points at product commit `339d00c`) carries `stokmate-v1.0.0-release.apk`; both READMEs and `docs/IMPLEMENTATION_REPORT.md` publish its SHA-256. APK packaging is **not** byte-reproducible — the same commit rebuilt yields a different checksum, so whenever you rebuild you must re-verify and re-publish the checksum everywhere it appears rather than assuming the recorded one still matches. Never rebuild between verifying an artifact and uploading it.
