...
- **Delivery is a public GitHub Release, not a local file.** The original `v1.0.0` release AND its tag were removed on 2026-09-01 by coordinator directive (record: `~/firstmate/data/release-cleanup-tr/old-release-record.md`); the authoritative `v1.0.0` will be recreated from the exact final commit with a freshly built, verified APK and a fresh SHA-256 — until then the release URL in the docs is intentionally labeled "pending recreation". APK packaging is **not** byte-reproducible — the same commit rebuilt yields a different checksum, so whenever you rebuild you must re-verify and re-publish the checksum everywhere it appears rather than assuming a recorded one still matches. Never rebuild between verifying an artifact and uploading it.
