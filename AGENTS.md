...
- Android on this machine: working AVD is `TripFlow_API_36` (`medium_phone` references a missing Play Store image); SDK at `~/Library/Android/sdk`; export `JAVA_HOME=/opt/homebrew/opt/openjdk@17` for Gradle (`/usr/libexec/java_home` cannot see it). Metro port 8081 is often taken by another project — use `npx expo start --port 8082`.
