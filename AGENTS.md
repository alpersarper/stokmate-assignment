...
- Android on this machine: working AVD is `TripFlow_API_36` (`medium_phone` references a missing Play Store image); SDK at `~/Library/Android/sdk`; export `JAVA_HOME=/opt/homebrew/opt/openjdk@17` for Gradle (`/usr/libexec/java_home` cannot see it). Metro port 8081 is often taken by another project — pass `--port 8082`. Mobile dev runtime is the **native debug build** (`npx expo run:android --port 8082` from `mobile/`); Expo Go has been uninstalled from the AVD and is no longer the primary dev runtime. The Metro port is baked into the debug build, so JS-only iteration is `npx expo start --port 8082` + relaunch the installed app: `adb shell am start -n com.stokmate.app/.MainActivity`. When attaching any custom debugger/DevTools client to Metro's `/inspector/debug`, use `127.0.0.1` URLs and send `Origin: http://127.0.0.1:<port>` — Expo CLI terminates (WS 1006 ~1ms after attach) debugger sockets whose Origin doesn't exactly match the dev-server base URL, and `localhost` does not match. `expo-dev-client` is installed (dev builds only — its Gradle wiring is `debugImplementation` + no-op release stubs), so the debug build's DevTools Expo Network panel captures app OkHttp traffic. After an emulator cold boot the system UI may throw a persistent "System UI isn't responding" ANR dialog that ignores taps — `adb root && adb shell pkill -f com.android.systemui` clears it. After `am force-stop`, launching `.MainActivity` lands on the expo-dev-launcher home screen (no custom URL scheme is registered, so dev-client deep links don't resolve) — tap the discovered dev-server row to load the bundle. On this AVD `adb shell input text` accepts Unicode, so Turkish characters can be typed into RN inputs directly.
- `npm run test:live --workspace shared`: the "single-flight refresh" assertion (`refreshCalls === 1`) can flake to 2 on this machine — a timing race in the live-network test (a 401 handled after the shared refresh resolves starts a second, still-safe refresh), not a client bug. Verified to flake on pristine main (2026-08-30).
...
- Mobile network-evidence trick: run a tiny logging proxy on the Mac (5081→5080, timestamped request log) and start Metro with `EXPO_PUBLIC_API_URL=http://10.0.2.2:5081` — every app request becomes greppable without DevTools. Kill/restart the proxy (not the backend) to simulate failures without wiping in-memory sessions.
- Emulator gesture measurement pitfalls (AVD): RN scroll events report dp, not px (density 2.625 → screen is 411×914dp); `input swipe` flicks with duration ≤120 ms sometimes register as a row TAP (navigates to detail — its ~10 s poll then pollutes the proxy log); swipes must START on the list area (below the ~750 px chrome), not on search/filters; returning to a stale cached dataset legitimately refetches its loaded pages sequentially (~100 ms apart) — don't misread that burst as a pagination defect. Scroll-gesture invariants for pagination live in the `ProductListScreen.tsx` guard comment.
- Headless-Chrome verification quirks: `chrome-devtools-axi screenshot` can only write inside the session scratchpad (sandboxed bridge) — capture there, then `cp`. DevTools "Offline" network emulation does NOT block localhost fetches, so web failure states also need the logging-proxy trick (below), not emulation. Tab switches do NOT fire `visibilitychange` (background tabs stay "visible" — polling continues); simulate hide/show by overriding `document.visibilityState` and dispatching `new Event('visibilitychange', { bubbles: true })` — TanStack's focusManager listens on `window`, so non-bubbling events are ignored. Failed (connection-refused) fetches don't appear in `performance.getEntriesByType('resource')`; use `chrome-devtools-axi network --type fetch` for those. The shared api-client captures `fetch` at module load, so patching `window.fetch` later observes nothing.
- RN Fast Refresh can leave text unpainted after large StyleSheet/JSX edits (layout correct, pixels missing) — don't debug the styles; force a full JS reload with `curl -X POST localhost:8082/reload`, and if still wrong cold-start the app via the dev launcher. Mobile design tokens now include spacing/type/elevation scales (`mobile/src/lib/theme.ts`); the surface language is documented in `mobile/DESIGN.md` — extend it, don't reintroduce outlined-box styling.
- Delivery: the authoritative GitHub Release `v1.0.0` was recreated 2026-09-01 from final commit `bd443f2` (asset `stokmate-v1.0.0-release.apk`, SHA-256 `becd9fa9c4f2…b4284`, verified end-to-end pre-upload; byte-identical local copy + full evidence in `~/firstmate/data/delivery/`). Release builds have no DevTools — to observe a release APK's traffic, `adb root` then in-emulator `iptables -t nat` DNAT of `10.0.2.2:5080` → `:5081` into a host logging proxy (the baked URL stays untouched); flush the rule and `adb unroot` afterwards.
- The repo is **public** and GitHub Pages is configured (deploy from branch `main`, folder `/docs`) → `https://alpersarper.github.io/stokmate-assignment/`. Reviewer reports live in `docs/reviewer/*.html` and the README links to their published URLs, so those filenames are part of the public contract — don't rename or move them. Every page must stay self-contained (embedded CSS, inline SVG, zero remote/CDN references) because reviewers also open them offline via `file://` from a clone; Pages only redeploys on pushes to `main`.
- **Docs are bilingual EN/TR and must stay that way.** `README.md` (English) and `README.tr.md` are equivalents linked by a selector line at the top of each; every `docs/**/*.html` page embeds both languages behind an `EN | TR` switch. Convention: `<html lang="en" data-lang="en">`, paired sibling blocks `<p lang="en">…</p><p lang="tr">…</p>` (whole `<tr>` rows for tables, never single `<td>`s), one `.langbar` switch as the first child of `.wrap`, and one inline vanilla `<script>` controller — no external JS ever. English is the default and must render correctly with JS disabled. `?lang=tr` deep-links work and the Turkish README uses them. Adding content to any page means adding BOTH languages. Reference implementation: `docs/index.html`. After editing, check in a browser that each language shows all of its own blocks and none of the other's — `display:none` on SVG `<text>` works but `offsetParent` does not, so measure SVG labels with `getComputedStyle`, not visibility helpers.
- **Delivery is a public GitHub Release, not a local file** (removal record of the original release: `~/firstmate/data/release-cleanup-tr/old-release-record.md`). APK packaging is **not** byte-reproducible — the same commit rebuilt yields a different checksum, so whenever you rebuild you must re-verify and re-publish the checksum everywhere it appears rather than assuming a recorded one still matches. Never rebuild between verifying an artifact and uploading it.
- UI polish: never restyle screens ad hoc — use the `/product-ui-polish` skill (`.claude/skills/product-ui-polish/`), which reads `DESIGN.md` (root + `web/` + `mobile/`, the recorded design language) and uses the project-scoped tooling in `.mcp.json` (chrome-devtools, shadcn) and `.claude/settings.json` (frontend-design plugin). Full system overview: `docs/ui-polish-system-impact.html`.
