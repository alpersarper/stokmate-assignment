 ...
 * (layer 2). Returns whether the trigger actually started a refresh so
 * callers driving native affordances (pull-to-refresh) can reflect it.
 * Even the narrow window where `isFetching` is stale in the closure is safe:
 * the joined refetch reuses the in-flight request.
 */
export function useManualRefresh(
  query: RefreshableQuery,
  cooldownMs: number = MANUAL_REFRESH_COOLDOWN_MS,
): { refresh: () => boolean; refreshDisabled: boolean } {
  const { refetch, isFetching } = query;
  const [coolingDown, setCoolingDown] = useState(false);

  const refresh = useCallback(() => {
    if (isFetching || coolingDown) return false;
    void refetch({ cancelRefetch: false }).finally(() => setCoolingDown(true));
    return true;
