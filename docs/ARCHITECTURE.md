...
- **Optional cross-client-refresh bonus (web, only after required work is stable):** ~15 s polling on the mounted list query + `refetchOnWindowFocus`. Zero backend change; `keepPreviousData` keeps it visually calm. Verified constraint: this reduces staleness windows only — nothing can prevent lost updates (§13). Since the data-freshness feature (`docs/DECISIONS.md` §13) the poll is not a free-running `refetchInterval` but a timer anchored to the last fetch settle, so it coordinates with the manual-refresh pipeline, focus/reconnect revalidation, and mutation invalidation instead of stacking on them; the same coordination applies to the mobile detail poll.
