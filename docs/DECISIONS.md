...
## 1. Minimal backend detail addition: `GET /products/{id}`

**Decision.** Add one read-only endpoint returning the full product, including `costPrice`, `supplierId`, `description`. This was the only backend addition needed to make the mandatory detail/edit flow lossless; the later Discontinued-stock rule and product-read limiter are separate, explicitly documented product/operational safeguards (§12–§13).
...
**Intent.** The app is almost entirely server state. Query keys per filter combination give stale-response protection for free; `keepPreviousData` satisfies the "keep existing results visible while loading" UX decisions natively. Web edits apply the response to detail and invalidate list datasets; mobile stock saves apply the canonical response directly to detail and every cached list row that already contains the product. Remaining client state is one auth context and local component state — no Redux/Zustand needed.
...
**Mitigations (UX-level only).** Edit forms initialize from a fresh `GET /products/{id}` and every mutation applies the server's returned state. Web product edits then invalidate list datasets; mobile stock saves patch existing detail/list cache entries without a success-path refetch. Freshness polling shortens staleness windows. This does not create concurrency protection, and a stock-sorted mobile dataset can remain out of order until its next explicit refresh because patching a row does not re-page the server-sorted result.
...
**Decision.** Every data surface (web list + detail, mobile list + detail) shows a subtle freshness indicator ("Updated just now / 24 sec ago / 3 min ago / Last updated at 14:32") driven by the time of the last *successful* fetch, plus an explicit cooldown-protected Refresh action. The web list polls its exact current server query every ~15 s while the page is focused/visible. Mobile detail polls only its current product every ~10 s while that screen is focused and the app is active; refocus/foreground triggers targeted revalidation. Manual refresh, mobile pull-to-refresh, lifecycle revalidation, and these polls all use the existing TanStack query for that key. Web edits invalidate lists; mobile stock success patches caches and does not refetch. The backend independently rate-limits only `GET /products`, `GET /products/{id}`, and `GET /products/stats` (60 requests / 10 s fixed window per authorization value, IP fallback → `429`; auth, lookups, and writes are excluded).

**Intent.** Refreshing should increase confidence in the data without disrupting work or creating avoidable backend load. TanStack Query's own primitives carry most of the weight: `dataUpdatedAt` is definitionally "last successful fetch" (a failed refresh cannot move the indicator), and joining in-flight fetches (`refetch({ cancelRefetch: false })`) makes single-flight + dedup structural rather than policed. The layered protection is: (1) single in-flight per query, (2) ~4 s client cooldown after a manual refresh, (3) query-level dedup of identical resource+params, (4) backend rate limiting as the independent non-client boundary, (5) bounded retries (retry ≤ 1, never on 4xx — a `429` is 4xx and is never auto-retried), (6) no stacking — both former `refetchInterval` polls were replaced by schedulers anchored to the last fetch *settle*, so a manual or lifecycle refetch pushes the next automatic poll a full interval out. A cache-only mobile stock success performs no fetch and therefore does not reset that scheduler.

**Boundaries.** Refresh preserves user context by construction (same query key → filters, search, sort, page, scroll, and unsaved drafts untouched; the existing snap-to-last-page and dirty-draft reconciliation paths handle invalidated state explicitly). Web polling stops while hidden; mobile detail polling stops while the screen is unfocused or the app is backgrounded. The backend limit is deliberately generous — legitimate polling plus cooldown-paced manual refreshes cannot approach it; only abusive request loops trip it. Contract details and runtime evidence: `docs/API_CONTRACT.md` §2a.
